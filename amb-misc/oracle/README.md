# AMB-misc / Oracle
Oracle responsible for listening to bridge related events and authorizing message relaying.
...
- sending transactions to authorize message relaying
...
There are two options to run the AMB processes:
...
| `HOME_MIN_AMOUNT_PER_TX` | Value (in _eth_ or tokens) to be sent in one transaction for the Home network. This should be greater than or equal to 500000000000000000, which is equivalent to 0.5. |
...
| `FOREIGN_MIN_AMOUNT_PER_TX` | Value (in _eth_ or tokens) to be sent in one transaction for the Foreign network. This should be greater than or equal to 500000000000000000, which is equivalent to 0.5. |
